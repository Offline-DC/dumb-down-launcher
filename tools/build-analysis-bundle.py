...
Expected input layout (after `adb pull` + `adb bugreport`):
    ...
    diag-<deviceid>-<date>/
      ...
      device-info-overrides.json           (optional — cohort, usage profile)
...
def build_hypotheses(
    ...
    return {
        "lid_sensor_chatter": {
            ...
            "comment": "Cross-check `lid_events_24h` against the control cohort. "
                       "If >> control median, suspect a chattering hall sensor.",
        },
...
def stub_device_info(diag_dir: Path) -> dict:
    ...
    fill in `cohort` and `usage_profile_self_report` by hand before sharing
    the bundle with the analyst.
    """
...
- `device-info.json` — hardware + cohort + usage profile. Fill in `cohort` and
  `usage_profile_self_report` by hand if `device-info-overrides.json` wasn't provided.
